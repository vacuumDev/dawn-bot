...
class FileOperations {
    constructor(basePath = './results') {
        ...
        this.modulePaths = {
            register: {
                success: path.join(this.basePath, 'registration', 'registration_success.txt'),
                failed: path.join(this.basePath, 'registration', 'registration_failed.txt'),
            },
            tasks: {
            ...
            accounts: {
                unverified: path.join(this.basePath, 'accounts', 'unverified_accounts.txt'),
                banned: path.join(this.basePath, 'accounts', 'banned_accounts.txt'),
                unregistered: path.join(this.basePath, 'accounts', 'unregistered_accounts.txt'),
    ...
    async setupStats() {
        ...
        for (const moduleName in this.modulePaths) {
            if (moduleName === 'stats') {
                ...
                const newPath = path.join(path.dirname(filePath), `accounts_stats_${timestamp}.csv`);
    ...
    async exportResult(result, module) {
        ...
        const filePath = this.modulePaths[module][result.status ? 'success' : 'failed'];
        ...
        try {
            ...
            console.error(`Account: ${result.identifier} | Error writing to file: ${error}`);
        } finally {
            this.unlockIfNeeded();
        }
    }
    ...
    async exportStats(data) {
        ...
        try {
            ...
            await csvWriter.writeRecords([{
                ...
                totalPoints: parseFloat(data.rewardPoint.points) + parseFloat(data.referralPoint.commission) + taskPoints,
