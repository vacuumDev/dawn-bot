        ...
        while (attempt < maxRetries) {
            try {
                const requestConfig = {
                    ...
                    headers: Object.keys(headers).length ? headers : { 'User-Agent': this.userAgent },
                    params,
                    data: jsonData,
                };
                ...
                if (attempt === maxRetries - 1) {
                    throw new Error(`Failed to send request after ${maxRetries} attempts: ${error.message}`);
                }
...
class DawnExtensionAPI extends APIClient {
        ...
        const jsonData = {
            ...
            referralCode:
                config.referralCodes && config.referralCodes.length > 0
                    ? config.referralCodes[Math.floor(Math.random() * config.referralCodes.length)]
                    : '',
            token: captchaToken,
            isMarketing: false,
            browserName: 'chrome',
    ...
    async completeTasks(appId, tasks = ['telegramid', 'discordid', 'twitter_x_id'], delay = 1000) {
        this._ensureAuthToken();
        const headers = {
            Authorization: `Bearer ${this.authToken}`,
            'User-Agent': this.userAgent,
